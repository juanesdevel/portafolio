...
if (isset($_POST['submit'])) {
    // Procesar el formulario enviado (crear nuevo registro)
    ...
    if ($resultado) {
        echo "<script> alert ('Nuevo registro creado correctamente'); location.assign ('mensualidades.php'); </script>";
    } else {
        echo "<script> alert ('ERROR: No se pudo crear el nuevo registro: " . mysqli_error($conexion) . "'); location.assign ('mensualidades.php'); </script>";
    }
} else {
    // Obtener datos del registro a partir del ID recibido
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
...
<body>
    ...
    <div class="container mt-5">
        <h2 style="color: #419cc7;">Crear Nuevo Registro</h2>
        <section>
            <div class="container">
                <form id="formulario" action="<?php echo $_SERVER['PHP_SELF'] . (isset($_GET['id']) ? '?id=' . $_GET['id'] : ''); ?>" method="post">
                    <div class="form-group">
                        <label for="entidad">Entidad:</label>
                        <input type="text" class="form-control" id="entidad" name="entidad" placeholder="Entidad" autocomplete="off" value="<?php echo $entidad; ?>" readonly required>
                    </div>

                    <div class="form-group">
                        <label for="dia_obligacion">Día Obligación:</label>
                        <input type="number" class="form-control" id="dia_obligacion" name="dia_obligacion" placeholder="Día Obligación" autocomplete="off" value="<?php echo $dia_obligacion; ?>" readonly  required>
                    </div>

                    <div class="form-group">
                        <label for="fecha_del_pago">Fecha del Pago:</label>
                        <input type="datetime-local" class="form-control" id="fecha_del_pago" name="fecha_del_pago" value="" required>
                    </div>

                    <div class="form-group">
                        <label for="valor_fijo">Valor Fijo:</label>
                        <input type="text" class="form-control" id="valor_fijo" name="valor_fijo" placeholder="Valor Fijo" autocomplete="off" value="<?php echo $valor_fijo; ?>" readonly required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="ref_contrato" name="ref_contrato" placeholder="Ref Contrato" autocomplete="off" value="<?php echo $ref_contrato; ?>" readonly required>
                    </div>
                    ...
                    <hr>
                    <button type="submit" class="btn btn-success" name="submit">Crear Registro</button>
                </form>
            </div>
        </section>
    </div>
