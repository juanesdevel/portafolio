...
if (isset($_POST['submit'])) {
    ...
    $codigo = trim($_POST['codigoUsuario']);
    $rol = trim($_POST['rolUsuario']);
    $nombre = htmlspecialchars(trim($_POST['nombreUsuario']));
    $password_raw = $_POST['password']; // Sin sanitizar, solo validar
    $password = password_hash($password_raw, PASSWORD_DEFAULT); // Hash seguro
    ...
    if (!preg_match('/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{4,12}$/', $password_raw)) {
    ...
    if (empty($errors)) {
        ...
        if ($stmt->execute()) {
            ...
            echo "<script>alert('ERROR: !Los datos no fueron ingresados a la base de datos¡'); location.assign('usuarios.php');</script>";
        }
        $stmt->close();
    } else {
