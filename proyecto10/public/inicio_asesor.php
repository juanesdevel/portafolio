...
if ($_SESSION['rol'] != 'asesor') {
    header("Location: ../public/inicio_admin.php"); // Redirigir a la página de inicio para otros roles
    exit();
}
...
<body>
    ...
    <nav class="navbar">
        ...
        <div class="navbar-user">
            ...
            <?php
if (isset($_SESSION['modulo_seleccionado'])) {
    $modulo_activo = $_SESSION['modulo_seleccionado'];
    echo "Módulo " . htmlspecialchars($modulo_activo);
    // Puedes utilizar $modulo_activo para controlar la lógica de tu aplicación
} else {
    echo "No se ha seleccionado ningún módulo en esta sesión.";
}
?>
</span>
        ...
        <div class="widget-container">
            <div class="container mt-5">
                ...
                <form method="post" class="mb-3">
                    ...
                    <button type="submit" class="btn btn-primary" name="filtrar_servicio">Filtrar</button>
                </form>

               <?php
    // Procesar el formulario si se ha enviado
    if (isset($_POST['filtrar_servicio'])) {
        $servicio_seleccionado = $_POST['servicio'];
        $sql_turnos = "SELECT id, fecha, entidad, turno, doc_usuario, atencion, estado FROM turnos WHERE servicio = ? AND (estado = 'Pendiente' OR estado = 'Notificado')";
        $stmt_turnos = $conn->prepare($sql_turnos);
        $stmt_turnos->bind_param("s", $servicio_seleccionado);
        $stmt_turnos->execute();
        $result_turnos = $stmt_turnos->get_result();
    } else {
        // Mostrar todos los servicios activos por defecto o un mensaje de selección
        $result_turnos = null;
    }
?>
<?php if (isset($result_turnos)): ?>
    <?php if ($result_turnos->num_rows > 0): ?>
        <table class="table table-striped">
            ...
            <tbody>
                <h3>Turnos de <?php echo htmlspecialchars($_POST['servicio']); ?></h3>
...
<?php elseif (isset($_POST['filtrar_servicio']) && $result_turnos === null): ?>
